...
router.get('/showDataContent/:article_id', async function(req,res){  //:id 이 아이디 갑ㅄ을 받아서, loaddata하는것.
	let data= [];
	let flag;
...
router.post('/showDataList',async function(req, res){  //여기서 받는 것. 데이터를. dataload.ejs에 도큐먼트가 레디다자 마자 post타입으로 데이터를 보냄.
	...
	if (results[0]){ //if true
		...
		data=[{'id': '',
			   'address': '',
			   'major': '',
			   'interests': ''}];    //내용이 없더라도 json에서 플래그랑 데이터 둘다 보내기 때문에 빈거라도 보냄.가짜로
		flag=0;
	}
...
const loadData = async(id)=>{
	try{
		...
		try{
			...
			if (id === null)
				select_query = "SELECT id,address,major,interests FROM join_db";   //id가 null이면 다 받아오고.
			else
				select_query = "SELECT id,address,major,interests FROM join_db WHERE id ='" + id +"'";   //id에 해당하는 값을 받아온다는 것.
